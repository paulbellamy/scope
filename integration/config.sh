...
IP_PREFIX=${IP_PREFIX:-192.168.49}
...
scope_on() {
    host=$1
    shift 1
    [ -z "$DEBUG" ] || greyly echo "Scope on $host:$DOCKER_PORT: $@" >&2
...
start_container() {
    host=$1
    shift 1
    docker_on $host run "$@" -t $SMALL_IMAGE /bin/sh
}
...
start_suite() {
    for host in $HOST1 $HOST2; do
        [ -z "$DEBUG" ] || echo "Cleaning up on $host: removing all containers"
